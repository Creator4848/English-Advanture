...
export default function Header() {
    ...
    return (
        <nav className="flex items-center justify-between px-6 lg:px-20 py-5 border-b border-gray-100 sticky top-0 bg-white/95 backdrop-blur z-50">
            ...
            <div className="hidden md:flex items-center gap-8 text-sm font-bold text-gray-500">
                ...
                {loggedIn ? (
                    <button
                        ...
                        className="text-sm font-bold text-gray-500 hover:text-[#111111] transition-colors"
                    >
                        Chiqish
                    </button>
                ) : (
            ...
            {/* Mobile CTA */}
            <Link href={loggedIn ? "/dashboard" : "/login"} className="md:hidden btn-yellow text-sm px-4 py-2">
                {loggedIn ? "Dashboard" : "Kirish"}
            </Link>
        </nav>
    );
}
